      ...
      <main className="flex px-20 py-7 bg-[#f5f5f5] h-[580px] gap-5">
        ...
        <div className="flex-[80%] flex flex-col items-center gap-3 pt-5 p-8 bg-white rounded-xl shadow overflow-y-auto">
          <div className="w-full mb-14">
            ...
            <div className="w-[340px] text-slate-500">Add at least one phone number for the transfer ID so you can start transfering your money to another user.</div>
          </div>
          {successMessage ? (
          ...
          <form onSubmit={updatePhoneNumber} className="flex flex-col gap-10 w-6/12">
            <div className={`flex gap-3 items-center border-b-2 ${contentPhoneNumber ? "border-[#10A19D]" : "border-[#A9A9A9]"}`}>
              <BsTelephone className={`text-xl ${contentPhoneNumber ? "text-[#10A19D]" : null}`} />
              ...
              <input
                ...
                name="phoneNumber"
                id="phoneNumber"
                placeholder="Enter your phoene number"
              />
    
            </div>
            <div>
              <button type="submit" className={`w-full rounded-xl font-bold py-3 ${contentPhoneNumber ? 'bg-[#10A19D] text-white': 'bg-[#E5E5E5]'}`}>Continue</button>
            </div>
          </form>
        </div>
      </main>
